...
# --------------------------------------------------------------------- #
COMMANDS = [
    {"id": "asistente", "icon": "robot",     "description": "Chat con el asistente"},
...
async def main(msg: cl.Message):
    ...
    if cmd == "asistente":
        ...
        await cl.Message("🤖 Modo asistente activado. ¡Escribe tu pregunta!").send()
        return
    ...
    # -------------------- FLUJO DE CHAT ASISTENTE ---------------------- #
    if mode == "assistant":
        user_email = await _require_email()
        response = await post_form(
            "/assistant",
            data={"input": msg.content, "email": user_email},
        )
